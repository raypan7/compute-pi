...
double compute_pi_leibniz_avx(size_t N)
{
    ...
    for (int i = 0; i <= N - 4; i += 4) {
        ...
        ymm3 = _mm256_mul_pd(ymm1, ymm3);   // 2*i, 2*(i+1), 2*(i+2), 2*(i+3)
        ymm3 = _mm256_add_pd(ymm3, ymm2);   // 2*(i+1), 2*(i+2), 2*(i+3), 2*(i+4)
        ymm3 = _mm256_div_pd(ymm0, ymm3);   // (-1)^i/(2*(i+1)), (-1)^i/(2*(i+2)), (-1)^i/(2*(i+3)), (-1)^i/(2*(i+4))
        ymm4 = _mm256_add_pd(ymm3, ymm4);   // pi += ymm3
    }
    double tmp[4] __attribute__((aligned(32)));
    _mm256_store_pd(tmp, ymm4);             // move packed float64 values to  256-bit aligned memory location
    pi += tmp[0] + tmp[1] + tmp[2] + tmp[3];
